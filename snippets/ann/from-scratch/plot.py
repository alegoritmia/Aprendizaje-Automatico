...
def plot_decision_boundary(X, y, classifier, theta):
    ...
    # yhat = classifier(np.column_stack((the_ones,grid)).T,theta) 
    yhat = classifier(grid.T,theta) 
    ...
    for class_value in unique_classes:
        ...
        row_ix = where(y_as_int == class_value)[0] # need the first element of the tuple :S
